...
def computeeigen(posbin, massbin, a2):
        ...
        for i in np.arange(3):
            M[i,i] = np.sum(massbin*posbin[:,i]**2/a2)
            for j in np.arange(i):
                temp = np.sum(massbin*posbin[:,i]*posbin[:,j]/a2)
                M[i,j] += temp
                M[j,i] += temp
        if type(massbin) is int:
            M /= float(len(posbin))
        else:
            M /= np.sum(massbin)
        ...
        temp = V.T.dot(M.dot(V))
        if not np.allclose(temp,np.diag(E),atol=1e-10):
            t(M.dot(V))
...
@jit(nopython=True)
def getbin(pos, q, s, rin, rout):
    d2 = pos[:,0]**2 + (pos[:,1]/q)**2 + (pos[:,2]/s)**2
    sel = (d2>rin**2) & (d2<rout**2)

    return d2,sel, pos[sel]

@jit(nopython=True)
def restrict(pos, rout):
    #d2 = np.sum(pos**2,axis=1)
    d2  = pos[:,0]**2 + (pos[:,1])**2 + (pos[:,2])**2
    sel = d2 < rout**2
...
def ellipsoidfit(posold, rvir, rin, rout, mass=None, weighted=False,\
    ...
    # Avoid particles at the center just in case
    #if rin == 0:
    #    rin = 1e-10
    ...
    for it in xrange(maxiter):
        ...
        if qall[it+1] < 1e-2:
            ...
            meanpos = np.average(posbin,axis=0)
            print meanpos, np.linalg.norm(meanpos)
            return -1.,-1.,0, np.zeros((3,3)),np.count_nonzero(d2< (rin**2))
...
def test(N=1000,h=0.1,R=1.,q=1.,s=1.,phi=0.,theta=0.,weighted=False,maxiter=150,verbose=True,convcrit=1e-3):
	...
	print 'Npart',N,len(pos)

	out=ellipsoidfit(pos,1.,0,1e10,convcrit=convcrit, weighted=weighted,maxiter=maxiter,verbose=verbose)
...
if __name__=="__main__":
    ...
    parser.add_argument("--npart", type=int, default=5000, help="Number of particles")
    ...
    print "8) Npart = 100000,,q=0.8,s=0.5,phi=30.,theta=30)"
    test(100000,q=0.8,s=0.5,phi=30.,theta=30.,h=0.9,R=2)

    print "\nREDUCED"
    print "9) q=0.9,s=0.7"
    test(N,q=0.9,s=0.7,weighted=True)

    print "10) q=0.5,s=0.25:"
    test(N,q=0.5,s=0.25,weighted=True)

    print "11) q=0.5,s=0.25,phi=45,theta=0"
    test(N,q=0.5,s=0.25,phi=45.,theta=0.,weighted=True)

    print "12) q=0.7,s=0.2,phi=45,theta=60"
    test(N,q=0.7,s=0.2,phi=45.,theta=60.,weighted=True)
